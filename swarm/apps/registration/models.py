...
class RegistrationManager(models.Manager):
    ...
    def activate_user(self, activation_key):
        ...
        """
        from swarm.apps.registration.signals import user_activated
    ...
    def create_user(self, first_name, last_name, password, email, send_email=True):
        """
    ...
    def create_username(self,first_name='',last_name=''):
        ...
        if not username:
            username = "amigeo"
        username = re.sub(r'\.|\s|\-|\W','',username)
        up = User.objects.filter(username__startswith=username).order_by('-id')
        if len(up) != 0:
            match = re.search(r'\d+$',up[0].username)
            if match and match.group(0):
                old_number = int(match.group(0))
                new_number = old_number + 1
                username = re.sub(r'\d+$',str(new_number),up[0].username)
            else:
                new_number = '1'
                username = username + new_number
        return username
